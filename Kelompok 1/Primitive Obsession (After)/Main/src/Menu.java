...
public class Menu {
	...
	private void viewChar() {
		if (cl.isEmpty()) {
			...
			System.out.println(
					"||  No  | Name \t\t| Class \t | Weapon \t\t | Weapon Type \t | Armor \t\t | Armor Type \t ||");
			System.out.println(
					"---------------------------------------------------------------------------------------------------------------------------");
			int count = cl.getSize();
			for (int i = 0; i < count; i++) {
				// kemudian ubah pemanggilan classnya
				System.out.printf("||  %-3d | %-13s | %-14s | %-21s | %-13s | %-21s | %-13s ||\n", i + 1, cl.getName(i),
						cl.getCharClass(i), cl.getWeapon(i).getWeapon(), cl.getWeapon(i).getWeaponType(),
						cl.getArmor(i).getarmor(),
						cl.getArmor(i).getarmorType());
			}
		}
	}
	...
	private void insertChar(String name, String charClass, String weapon, String weaponType, String armor,
			String armorType) {
